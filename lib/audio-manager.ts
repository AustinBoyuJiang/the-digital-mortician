export class AudioManager {
  ...
  async playBGMusic() {
    if (this.isBGMusicPlaying || !this.audioContext) return;
    ...
    const chord1 = [220, 277.18, 329.63];
    const chord2 = [246.94, 311.13, 369.99];
    const chord3 = [196, 246.94, 293.66];
    const chord4 = [164.81, 207.65, 246.94];
    ...
    const playChord = (chord: number[], startTime: number) => {
      ...
      chord.forEach((freq, index) => {
        ...
        noteGain.gain.linearRampToValueAtTime(0.12, startTime + chordDuration - 0.5);
      ...
      bassGain.gain.linearRampToValueAtTime(0.18, startTime + 0.1);
      bassGain.gain.linearRampToValueAtTime(0.18, startTime + chordDuration - 0.3);
    ...
    const playLoop = () => {
      ...
      setTimeout(playLoop, chordProgression.length * chordDuration * 1000);
    };
